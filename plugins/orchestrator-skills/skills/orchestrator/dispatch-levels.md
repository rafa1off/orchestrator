...
A **teammate** (L3b) is a Claude Code team-mode agent (gated by `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`) — a mini-orchestrator that owns its own context window, runs its own read → write → verify → test loop, spawns its own subagents, and exchanges messages with peers. It is a distinct top-level role: the no-`Agent` rule above governs the 8 when dispatched as subagents and does not constrain the team-mode orchestrator.

**The 8 may be dispatched as teammates — it is a cost question, not a prohibition.** Two properties used to make it silently lossy, and both are now closed, so what remains is economics rather than correctness.

*Delivery.* The two dispatch modes have incompatible return channels. A subagent's final message *is* the `Agent` tool result; a teammate is an independent session with no tool result, so its final message is delivered to nobody and only `SendMessage` crosses the boundary — the `Teammate @x finished` line is a lifecycle event carrying no payload. Every definition now carries a `## Delivery` section that sends over `SendMessage` when it detects teammate dispatch, and a `TeammateIdle` guard refuses the idle when it doesn't.

*Enforcement.* `agent_type` is the definition type for a subagent but the teammate's **name** in team mode, so guards matching the definition type used to no-op on every teammate. They now resolve both shapes through `lib-resolve-agent-identity.sh`, and `TeammateIdle` carries the findings requirement that `SubagentStop` cannot reach in team mode.

**The remaining cost is real, though.** A teammate is a full Claude Code session: its own context window, its own model calls, no result summarisation back into yours. Handing one a leaf job — map these files, run lint — spends a whole session on work a subagent does for a fraction of it, and the output still has to be messaged back by hand. Prefer a subagent whenever the job is "take this context block and return a result".
...
| One of the 8 as a teammate for a leaf job | L3b reader/checker teammate | Subagent (L1/L2) | Allowed, but a teammate is a whole session — leaf work gets a fraction of that from a subagent, and its result returns automatically |
